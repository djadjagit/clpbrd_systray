...
SendMessage(FNextViewer, WM_DRAWCLIPBOARD, 0, 0);
 ...
 if Clipboard.HasFormat(CF_BITMAP) then
  begin
   ...
   pm1.Items.Insert(0,NewItem('Картинка', 0, False, True, N1Click, 0, 'N'+inttostr(k2)));
//   pm1.Items.Add(NewItem('Картинка', 0, False, True, N1Click, 0, 'N1'));
//   pm1.Items.Items[pm1.Items.Count-1].Bitmap:=Image1.Picture.Bitmap;
   ...
   TrayIcon1.Hint:='Запись:"Картинка" сохранена. Время:'+timetostr(now());
  end
   else
   if ClipBoard.AsText<>'' then
    ...
    //   pm1.Items.Add(NewItem(s, 0, False, True, N1Click, 0, 'N1'));
       ...
       TrayIcon1.Hint:='Запись:"'+s+'" сохранена. Время:'+timetostr(now());
     end;
 if pm1.Items.Count>30 then pm1.Items.Delete(pm1.Items.Count-3);
 except
 TrayIcon1.Hint:='Возникла ошибка при сохранении буфера обмена. Время:'+timetostr(now());
 form1.Caption:='Ошибка';
 end;
end;
form1.Tag:=2;
end;
...
if AdoTable1.Locate('key_all',nom,[loCaseInsensitive]) then
 begin
  ...
  TrayIcon1.Hint:='Запись:"'+s+'" скопирована в буфер обмена. Время:'+timetostr(now());
 end
  else
  if AdoTable2.Locate('key_all',nom,[loCaseInsensitive]) then
   begin
    ...
    TrayIcon1.Hint:='Запись:"Картинка" скопирована в буфер обмена. Время:'+timetostr(now());
   end
except
TrayIcon1.Hint:='При копировании записи:"'+nom+'" в буфер обмена возникла ошибка. Время:'+timetostr(now());
form1.Caption:='error';
end;
end;
   ...
   if ADOTable1.RecNo>0 then
    ...
    begin
     ...
     if ADOTable2.RecNo>0 then
       begin
         pm1.Items.Add(NewItem('Картинка', 0, False, True, N1Click, 0, 'N'+ADOTable2.FieldByName('key_all').AsString));
...
pm1.Items.Add(NewItem('Выход', 0, False, True, D1Click, 0, 'D1'));
